...
def _sync_save_to_excel(profile: UserProfile, name_sys: str = "") -> None:
    """
    ...
    try:
        ...
        # Проверяем на дубликаты: ищем последнюю запись с таким же tg_user_id (в колонке D - Name_sys может содержать ID)
        # Или проверяем по комбинации статуса, имени и телефона
        is_duplicate = False
        if worksheet.max_row > 1:  # Если есть данные кроме заголовков
            # Проверяем последние несколько строк (например, последние 5) на дубликаты
            # Дубликат = та же комбинация статуса, имени и телефона
            check_rows = min(5, worksheet.max_row - 1)  # Проверяем последние 5 строк или все, если меньше
            for row_idx in range(worksheet.max_row, max(1, worksheet.max_row - check_rows), -1):
                try:
                    ...
                    # Считаем дубликатом, если статус, имя и телефон совпадают
                    if (existing_status == new_status and 
                        existing_name == new_name and 
                        existing_phone == new_phone and
                        new_status and new_name and new_phone):  # Только если все три поля заполнены
                        is_duplicate = True
                        logger.info(
                            f"⚠️ Обнаружен дубликат в строке {row_idx}: "
                            f"статус='{new_status}', имя='{new_name}', телефон='{new_phone}'. Пропускаем добавление."
                        )
                        break
                except Exception as e:
...
async def save_lead_to_excel(profile: UserProfile, name_sys: str = "") -> None:
    ...
    try:
        ...
        await loop.run_in_executor(None, _sync_save_to_excel, profile, name_sys)
