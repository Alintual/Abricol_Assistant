...
def is_rule_intent(query: str) -> bool:
    ...
    return any(word in lowered for word in RULE_INTENT_KEYWORDS)
...
def _move_cta_to_end(text: str) -> str:
    """Переносит вопросы и call-to-action в конец, сохраняя естественный порядок."""
    if not text:
        return text
    ...
    for sentence in sentences:
        ...
        if has_cta_keyword or is_question:
            cta_sentences.append(stripped)
        else:
            other_sentences.append(stripped)
...
def _validate_anketa_answer(answer: str, question_num: int) -> tuple[bool, str]:
    ...
    if question_num == 1:  # Опыт игры
        ...
        level_keywords = [
            ...
            "опытн", "неопытн", "умею", "не умею", "знаю", "не знаю"
        ]
        ...
        goals_keywords = [
            ...
            "техник", "играть", "игра", "бильярд", "пирамида", "турнир", "соревнован"
        ]
...
def _enhance_layout(text: str) -> str:
    ...
    # Для нумерованных списков в стиле '1. ' или '2) ' слипающихся — всегда новая строка
    text = re.sub(r'(\S)\s*(\d+[.)])', r'\1\n\2', text)

    # Старые правила
    text = re.sub(r"(\S)\s*👉", r"\1\n👉", text)
    text = re.sub(r"^\s*👉", "👉", text, flags=re.MULTILINE)
    text = re.sub(r"\s*([🧿🔹▶️🔸✓➡️])", r"\n\1", text)
    text = re.sub(r"(\n|^)\s*- ", r"\1- ", text)
    text = re.sub(r"(\n|^)\s*• ", r"\1• ", text)
...
async def _show_intent_selection_window(
    ...
    try:
        ...
        text = (
            ...
            "👉 - Записаться на Обучение [О]\n"
            "👉 - Записаться на Консультацию по телефону [К]\n"
            "👉 - Продолжить общение в чате [П]"
        )
...
Инструкция: Ответь на вопрос клиента на основе ПРЕДОСТАВЛЕННЫХ КОНТЕКСТОВ. Следуй системному промпту (ты Леонидыч, ассистент школы бильярда «Абриколь»). Используй ТОЛЬКО информацию из контекстов выше. Если информации недостаточно, скажи об этом. Отвечай кратко, естественно, как живой человек."""
    ...
    # Приоритет специфичным ключевым словам (технич, размер, требован) над общими (правила)
    # Сначала проверяем специфичные ключевые слова для технических требований
    technical_keywords = ["технич", "размер", "требован"]
    ...
    if not matched_corpus_from_alias:
    ...
    if has_rules_sources_in_hits and detected_topic == "school":
    ...
    if matched_corpus_from_alias and matched_corpus_from_alias in RULE_PRIMARY_ALLOWED_SOURCES:
    ...
    # Если запрос относится к правилам (rule_query=True), кнопка должна показываться независимо от наличия документов "О школе"
        ...
        try:
            ...
            if allowed_sources:
                ...
                elif not primary_sources_blocked and rule_query:
    ...
    # 2. Нет стоп-слов в ответе LLM (НО для правил это не блокирует кнопку)
    # 3. И (stored_primary_sources не пуст ИЛИ rule_query=True с fragment_sources из правил)
    # Для правил (rule_query=True) игнорируем блокировку из-за стоп-слов
    should_show_button = allow_rule_button and (
